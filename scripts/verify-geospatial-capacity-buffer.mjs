...
const fail = (message) => {
  throw new Error(`[geospatial-capacity] ${message}`);
};
const requireText = (token, label = token) => {
  if (!source.includes(token)) fail(`Missing ${label}`);
};

if (!extension.includes("geospatial-capacity-buffer.js"))
  fail("Capacity-buffer extension is not mounted");
...
if (effective !== 5100)
  fail("6000 × 85% must produce 5100 effective planning capacity");
if (!(effective < physical)) fail("Capacity buffer must reserve physical headroom");

console.log(
  "[geospatial-capacity] PASS: editable physical capacity, 85% default utilisation buffer and effective planning capacity checks passed.",
);
