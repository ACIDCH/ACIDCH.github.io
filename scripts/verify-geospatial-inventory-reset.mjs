...
const source = fs.readFileSync(
  path.join(root, "src/scripts/geospatial-inventory-variability.js"),
  "utf8",
);
const fail = (message) => {
  throw new Error(`[geospatial-inventory-reset] ${message}`);
};
...
if (Math.abs(effective - 25) > 1e-9)
  fail("Fixed-lead-time baseline must restore the raw demand SD of 25");

console.log(
  "[geospatial-inventory-reset] PASS: mean demand, demand SD, lead time, lead-time SD, 95% service, holding cost and effective SD reset synchronously to baseline.",
);
