...
## 冗余约束不会改变可行域，但会影响模型可读性

有些约束在现有规则下永远不会成为限制条件。例如已经有：
...
第二条在当前结构中是 redundant constraint，因为任何满足 `x ≤ 80` 的方案自然也满足 `x ≤ 100`。

冗余约束不一定导致错误，有时为了业务可读性仍值得保留。例如合同文件明确写着“单周不得超过 100”，即使另一条更紧的设备约束已经限制到 80，保留合同约束仍有审计价值。

但大量无意义冗余会增加模型规模，也会让诊断结果更难阅读。因此可以区分：
...
建模时应知道每条约束为什么存在，而不是因为“多写一条更保险”。

## Equality 与数值容差需要区分数学要求和求解器实现
...
数值求解器内部使用浮点计算，因此实际检查常存在很小的 feasibility tolerance。结果可能出现类似：
...
这不代表业务规则被放弃，也不应该用手工四舍五入去掩盖真正的较大违约。结果验证可以设置一个明确容差，例如：
...
如果出现 498 对 500，则不是浮点误差，而是模型、数据或求解状态需要重新检查。
...
## Infeasible 诊断应该逐层缩小冲突范围

面对不可行模型，最危险的做法是随机删除约束直到模型能跑。
...
4. 检查 equality 是否过强
5. 按约束族逐组启用或禁用进行定位
6. 如果 solver 支持，再查看 IIS / conflict information
```

例如总需求 860，而全部设施最大容量只有 800，在不允许 shortage 或外部采购时，已经可以在求解前证明模型不可行。

这种 pre-solve sanity check 比等待求解器返回 `Infeasible` 更高效，也更容易向业务解释：问题不是算法失败，而是当前承诺和资源无法同时成立。
