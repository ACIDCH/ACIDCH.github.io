...
title: 表关系
summary: 两张表怎么关联，关键看一条记录最多能对应多少条另一张表的记录。这里用客户、订单和产品说明一对多、多对多和一对一，以及 JOIN 后为什么会多出行。
tags:
  - 一对多
  - 多对多
  - 一对一
...
外键能说明一条记录引用了另一条记录，但建表时还要继续问两个问题：
...
答案决定表之间的关系基数，也就是 Relationship Cardinality。
...
这不是三种 SQL 语法，而是三种业务关系。数据库只是把这种关系写进表结构。
...
客户 1002 在当前样本里只有一张订单，并不代表关系变成了 1:1。关系基数看的是业务允许发生什么，而不是眼前这几行数据刚好出现了什么。

### 外键通常放在“多”的一边

如果一张订单只能属于一个客户，订单表最适合保存 `customer_id`：
...
`orders.customer_id` 可以重复，因此同一个客户能对应多张订单。

反过来说，“多张订单属于一个客户”就是 Many-to-One，它和 One-to-Many 是同一段关系从两个方向看。

判断 1:N 时，直接把业务规则说出来通常最清楚：
...
如果只在 `orders` 里加一个 `product_id`，一张订单只能直接指向一个产品；如果把 `order_id` 放进 `products`，同一个产品又只能指向一张订单。两种做法都装不下真实的 N:M 关系。
...
叫法不同，作用相同：专门保存两类实体之间的关系。

## 中间表为什么经常使用联合主键？

当前规则规定，同一张订单里的同一个产品只保留一条明细。因此：
...
并不是所有中间表都一定要用联合主键。如果业务允许同一产品在一张订单里分成多条记录，就需要换一种主键设计。关键仍然是一行到底代表什么。

### 中间表还能保存关系本身的信息

`quantity` 和 `unit_price` 很适合放在 `order_items`，因为它们既不是订单整体的固定属性，也不是产品主数据的固定属性，而是“这个产品出现在这张订单里”时才产生的信息。
...
这种回算很重要。中间表一旦成为分析数据源，明细合计和订单总额是否一致应该可以被检查，而不是默认它们一定一致。
...
`customer_id` 同时是 PK 和 FK，所以在 `customer_profiles` 中天然不会重复。
...
所以“一对一”不是更高级的默认设计。拆不拆表，要看是否真的有业务理由。
...
这里 `customer_id` 重复并不是查询错了，而是 1:N 展开后的正常结果。

真正需要检查的是：新增行数是否符合原本的业务关系，还是 JOIN 条件写错造成了额外放大。
...
很多 JOIN 错误并不是语法错误，而是查询跑通了，粒度却已经改变。
...
先把业务规则说清楚，再决定表结构，通常比先画几张表再猜它们是什么关系更稳妥。
...
表关系不是看当前数据“像什么”，而是看业务规则允许一条记录对应多少条另一类记录。接下来进入 `SELECT`，开始真正从这些表里读取数据。
