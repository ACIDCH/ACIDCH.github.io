...
title: 主键
summary: 一张表里可以有很多看起来唯一的字段，但真正适合当“记录身份证”的字段还要足够稳定。这里用客户表说明主键、自然键、自增 ID、UUID 和联合主键。
tags:
  - 主键
  - 关系模型
  - 数据完整性
...
这里 `customer_id` 承担的就是这种稳定身份。

主键（Primary Key）不是“最重要的业务字段”，而是数据库用来唯一定位一条记录的键。
...
`customer_id` 是主键。即使 `email` 也设置了 `UNIQUE`，两者职责仍然不同：邮箱是业务属性，客户 ID 是记录身份。
...
当前确实都不同。但客户换邮箱是一件很普通的业务变化。如果邮箱就是主键，更新联系方式就同时变成了更新记录身份。
...
这也是为什么“客户名称”一类经常修改的字段不适合承担主键身份。
...
这种约束的价值就在于：错误数据不是等分析时才发现，而是在写入数据库的那一刻就被阻止。
...
主键本身通常只有一列或几列，却是关系模型后面所有引用关系的基础。下一篇会继续处理外键：**一条记录有了稳定身份以后，另一张表怎样安全地引用它。**