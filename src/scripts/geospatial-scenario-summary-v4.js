...
function boot() {
  ...
  const copy = zh
    ? {
        ...
        robust: "稳健性变化（B − A）",
        robustUnavailable: "只有 A、B 都保存了当前 Monte Carlo 结果时，才比较稳健性指标。",
        objectiveWarning:
        ...
        trade: {
          ...
          similar: "A 与 B 在当前核心成本和网络 KPI 上接近，可进一步比较覆盖、库存与 Monte Carlo 稳健性。",
    ...
    : {
        ...
        objectiveWarning:
          "A and B use different optimisation objectives. The outcomes can still be compared as scenarios, but their difference should not be attributed only to parameter changes under a like-for-like objective.",
        engineWarning:
  ...
  function currentOpenFacilities() {
    return [...D.querySelectorAll("#geo4-open-list strong")]
  ...
  function captureSnapshot() {
    ...
    return {
      ...
      openFacilities: currentOpenFacilities(),
      labels: {
        objective: selectedText("geo4-objective"),
        engine: selectedText("geo4-engine"),
        roadMode: selectedText("geo4-road-mode"),
        service: selectedText("geo4-service"),
      },
    };
  }
  ...
  const objectiveLabel = (value) => {
    if (value === "minCost") return zh ? "最低总成本" : "Minimum total cost";
    return zh ? "最少设施" : "Minimum facilities";
  };
  const engineLabel = (value) =>
    value === "osm" ? (zh ? "OSM 道路网络" : "OSM Road Network") : (zh ? "快速 OD 网络" : "Fast OD Network");
  ...
  function formatParameter(key, value) {
    ...
    if (["congestion", "congestionShare", "closure"].includes(key)) return `${Number(value).toFixed(1)}%`;
    if (key === "threshold") return `${Number(value).toFixed(1)} ${slots.A?.params?.engine === "osm" || slots.B?.params?.engine === "osm" ? "min" : "km"}`;
    if (key === "redundancy") return `${value}×`;
    if (["leadTime", "leadTimeSd"].includes(key)) return `${Number(value).toFixed(1)} d`;
    if (key === "service") {
      const serviceMap = { "1.282": "90%", "1.645": "95%", "1.960": "97.5%", "2.326": "99%" };
      return serviceMap[String(value)] || String(value);
    }
    if (["fixedCost"].includes(key)) return cash(Number(value));
  ...
  function slotCard(slot, snapshot) {
    ...
    const metricValue = snapshot.metrics.networkValue;
    const facilityNames = snapshot.openFacilities.length
      ? snapshot.openFacilities.slice(0, 3).map(escapeHtml).join(" · ") +
        (snapshot.openFacilities.length > 3 ? ` +${snapshot.openFacilities.length - 3}` : "")
      : "—";
    return `<div class="geo4__ab-slot" data-slot="${slot}"><div class="geo4__ab-slot-head"><strong>${slot}</strong><span>${copy.saved}</span></div><small><b>${escapeHtml(engineLabel(snapshot.params.engine))}</b> · ${escapeHtml(roadLabel(snapshot.params.roadMode))} · ${escapeHtml(objectiveLabel(snapshot.params.objective))}<br>${cash(snapshot.metrics.totalCost)} · ${Number.isFinite(metricValue) ? `${metricValue.toFixed(2)} ${metric.unit}` : "—"} · ${Number.isFinite(snapshot.metrics.coverage) ? `${snapshot.metrics.coverage.toFixed(1)}%` : "—"}<br>${escapeHtml(facilityNames)}</small></div>`;
  }
  ...
  function coverageSentence(value) {
  ...
  function facilitySentence(value) {
  ...
  function interpretation(result) {
    const base = copy.trade[result.tradeoff] || copy.trade.similar;
    return `${base}${coverageSentence(result.deltas.coverage)}${facilitySentence(result.deltas.facilities)}`;
  }
  ...
  function renderChanged(result) {
    ...
    const rows = result.changedParameters
      .map(
        ({ key, a, b }) =>
          `<div class="geo4__ab-change"><span>${escapeHtml(parameterLabels[key] || key)}</span><strong>${escapeHtml(formatParameter(key, a))} → ${escapeHtml(formatParameter(key, b))}</strong></div>`,
      )
      .join("");
